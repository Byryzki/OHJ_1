...
class Car:
    ...
    def fill(self, to_fill):
        ...
        elif (self.__gas + to_fill) >= self.__tank_volume:
            self.__gas = self.__tank_volume

        else:
            self.__gas = self.__gas + to_fill

    def drive(self, distance):
        if distance < 0:
            print("You cannot travel a negative distance")
        else:
            theory = (self.__gas/self.__consumption) * 100

            if theory < distance:
                self.__odometer = theory + self.__odometer
                meno = (theory * self.__consumption) / 100
                self.__gas = self.__gas - meno

            else:
                self.__odometer = distance + self.__odometer
                meno = (distance * self.__consumption) / 100
                self.__gas = self.__gas - meno
